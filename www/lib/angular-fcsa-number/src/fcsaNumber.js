...
(function() {
  ...
  fcsaNumberModule.directive('fcsaNumber', [
    'fcsaNumberConfig', function(fcsaNumberConfig) {
      var addCommasToInteger, addDecimalsToNumber, controlKeys, defaultOptions, getOptions, hasMultipleDecimals, isNotControlKey, isNotDigit, isNumber, makeIsValid, makeMaxDecimals, makeMaxDigits, makeMaxNumber, makeMinNumber, trimLeadingZeros;
      ...
      trimLeadingZeros = function(val) {
        return val.replace(/^0+(?!\.|$)/, '');
      };
      addDecimalsToNumber = function(val, padNum) {
        var decimals, padString, parts;
        if (val === '') {
          return '';
        }
        parts = val.split('.');
        padString = new Array((parseInt(padNum, 10) || 2) + 1).join('0');
        decimals = decimals = ((parts[1] || '') + padString).match(/^\d\d/)[0];
      ...
      return {
        ...
        link: function(scope, elem, attrs, ngModelCtrl) {
          ...
          ngModelCtrl.$formatters.push(function(val) {
            ...
            ngModelCtrl.$setValidity('fcsaNumber', true);
            if (options.trimLeadZero != null) {
              val = trimLeadingZeros(val.toString());
            }
            if (options.padDecimal != null) {
              val = addDecimalsToNumber(val.toString(), options.padDecimal);
            }
